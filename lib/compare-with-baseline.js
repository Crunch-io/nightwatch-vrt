	...
	return new Promise((resolve, reject) => {
		getBaselineScreenshotOrCreate(
			...
			const diff = Jimp.diff(screenshot, baseline),
				identical =
					diff.percent <= (Number.isFinite(threshold) ? threshold : 0.0);

			if (!identical && prompt === true) {
				saveScreenshot(completeLatestPath, screenshot)
					.then(() => saveScreenshot(completeDiffPath, diff.image), resolve)
					.then(
						() =>
							promptScreenshotOverride(
								completeLatestPath,
								completeBaselinePath,
								completeDiffPath
							),
					...
					.then(
						() =>
							overrideBaseline(completeLatestPath, completeBaselinePath).then(
								() => cleanupScreenshots(completeLatestPath, completeDiffPath),
								resolve
							),
						resolve
					)
				...
				nightwatchClient.assert.ok(
					true,
					"Settings enforced overriding baseline screenshot."
				); // eslint-disable-line max-len
				saveScreenshot(completeBaselinePath, screenshot)
					.then(() => {
						resolve(true);
					})
					.catch(() => resolve(false));
				...
				saveScreenshot(completeLatestPath, screenshot)
					.then(() => saveScreenshot(completeDiffPath, diff.image), resolve)
					.then(
						() => (completeLatestPath, completeBaselinePath, completeDiffPath),
						resolve
					)
					.then(resolve);
			} else if (identical) {
				// Cleanup here in case user fixed bug and reran tests
				cleanupScreenshots(completeLatestPath, completeDiffPath).then(() => {
					resolve(identical);
				});
			} else {
				resolve(identical);
