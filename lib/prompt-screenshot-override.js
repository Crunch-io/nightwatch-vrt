...
function error_screenshots_differ(latestPath, expectedPath, diffPath) {
 ...
 ${RED}├${RESET}     ${RED}Latest${RESET}: file://${latestPath.replace(
		/ /g,
		"%20"
	)}
 ${RED}├${RESET}   ${GREEN}Expected${RESET}: file://${expectedPath.replace(
		/ /g,
		"%20"
	)}
 ${RED}└${RESET} ${YELLOW}Difference${RESET}: file://${diffPath.replace(
		/ /g,
		"%20"
	)}
